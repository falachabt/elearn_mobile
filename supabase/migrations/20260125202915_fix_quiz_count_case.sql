...
CREATE TRIGGER trig_sync_quiz_to_secondary_program AFTER INSERT OR DELETE ON public.quiz_courses FOR EACH ROW EXECUTE FUNCTION sync_quiz_to_secondary_program();

CREATE TRIGGER trig_recalc_quiz_count_on_course AFTER INSERT OR DELETE ON public.secondary_program_courses FOR EACH ROW EXECUTE FUNCTION recalc_program_quiz_count_on_course_change();

-- Recalculer immédiatement tous les quiz_count existants
DO $$
DECLARE
    v_program RECORD;
    v_quiz_count INTEGER;
BEGIN
    FOR v_program IN SELECT id FROM public.secondary_programs
    LOOP
        -- Compter les quiz uniques via les cours du programme
        SELECT COUNT(DISTINCT qc."quizId") INTO v_quiz_count
        FROM public.secondary_program_courses spc
        JOIN public.quiz_courses qc ON qc."courseId" = spc.course_id
