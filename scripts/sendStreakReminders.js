  ...
  try {
    ...
    const messages = usersWithTokens.map(user => {
      ...
      const personalizedBody = user.firstname 
        ? `${user.firstname}, votre série de ${hoursRemaining} jours consécutifs va expirer dans moins de ${hoursRemaining} heures. Connectez-vous maintenant pour la maintenir!` 
