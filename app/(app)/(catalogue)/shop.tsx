...
export default function ShopPage() {
    ...
    const fetchCourses = async () => {
        try {
            const {
                data: userProgarm,
                error: errorUserProgarm
            } = await supabase.from("user_program_enrollments").select(`program_id`).eq("user_id", user?.id);
            ...
            const {data, error} = await supabase
                ...
                .select(`
          *,
          learning_path:learning_paths(id, title, description, course_count, quiz_count),
